...
public class ChatBot4 {
    ...
    private int scheldcounter = 0;


    ChatBot4(String name) {
        this.name = name;
        ...
        this.scheldwoorden = new String[]{"idioot", "sukkel"};
    ...
    int getScheldCounter() {
        return scheldcounter;
    }

    String antwoordOPVraagV4(String vraag, int count) {
        int check = -1;
        boolean scheldCheck = false;
        String annoyed = "Zucht, typisch. ";
        ...
        for (String s : scheldwoorden) {
            if (vraag.contains(s)) {
                scheldcounter++;
                scheldCheck = true;
            }
        }
        if (check >= 0 && !scheldCheck) {
            return builder.append(antwoorden[check]).toString();
        } else if (scheldCheck) {
            return builder.append("Gelieve beleefd te blijven").toString();
        } else {
            return builder.append(nonsensAntwoorden[random.nextInt(nonsensAntwoorden.length)]).toString();
        }
    }
